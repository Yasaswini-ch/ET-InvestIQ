...
import { TrendingUp, TrendingDown, Minus, Info, Calendar, ChevronLeft, Search, Filter, Radar, Bookmark, X, RefreshCw } from 'lucide-react';
...
export default function RadarPage() {
  ...
  const filteredSignals = useMemo(() => {
    ...
    if (selectedConviction.length > 0) signals = signals.filter((s: any) => selectedConviction.includes(s.conviction));
                              ...
                              <button key={v} onClick={() => toggleConviction(v)}
                                className={`px-3 py-1.5 rounded-lg text-xs font-bold border transition-all ${
                                  selectedConviction.includes(v)
                                    ? v === 'high' ? 'bg-emerald-500/10 border-emerald-400/20 text-emerald-400' : 'bg-yellow-500/10 border-yellow-400/20 text-yellow-400'
